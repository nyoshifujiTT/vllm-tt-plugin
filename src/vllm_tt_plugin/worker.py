...
class TTWorker(WorkerBase):
    ...
    def determine_available_memory(self) -> int:
        ...
        # budget to reconstruct. Checked before the sizing assertion below: the
        # engine never asks a pooling model for KV memory, and get_kv_cache_spec
        # returns {} for one, so falling through would only build an empty spec.
        if isinstance(self.model_runner, TTPoolingModelRunner):
            return 0
